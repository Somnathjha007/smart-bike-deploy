...
const GUARDIANS_FILE = path.join(__dirname, 'data', 'guardians.json');
...
// POST endpoint to receive data from simulator
app.post('/api/bike/data', async (req, res) => {
  try {
    ...
    const enrichedData = {
      ...data,
      timestamp: now.toISOString(), // Use server-generated timestamp
...
app.get('/api/guardian/me', authenticateJWT, async (req, res) => {
  try {
    ...
    if (guardian) {
      ...
      // Create new guardian entry for user
      const newGuardian = {
...
app.post('/api/guardian/wards', authenticateJWT, async (req, res) => {
  try {
    ...
    // Generate new ward and bike IDs
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`🚴 Smart-Cycle Server running on port ${PORT}`);
  console.log(`📡 WebSocket server ready for real-time connections`);
  console.log(`💾 Data will be saved to: ${DAILY_DIR}`);
  console.log(`🌐 Accessible on local network at: http://localhost:${PORT}`);
});
